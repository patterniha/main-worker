...
export default {
	 ...
	 * @param {{ws_path: string, rd_path: string, dh_path: string, vless_uuid: string}} env
	 * @param ctx
	...
	async fetch(request, env, ctx) {
		try {
			...
			switch (req_path) {
				case env.rd_path:
					const search_params = parsed_request_url.searchParams;
					if (!search_params.has('fu')) {
						return ret_on_err;
					}
					const redirectUrl_b64 = search_params.get('fu');
					if (!redirectUrl_b64) {
						return ret_on_err;
					}
					const decoder = new TextDecoder();
					return await fetch(decoder.decode(base64ToArrayBuffer(redirectUrl_b64)), request);
				case env.dh_path:
