...
export default {
	 ...
	 * @param {{ws_path: string, rd_path: string, ah_name: string, ah_value: string, vless_uuid: string, rd_full_url_hname: string, padding_hname: string}} env
	 * @param ctx
	...
	async fetch(request, env, ctx) {
		try {
			...
			const s_ah_name = env.ah_name;
			const req_ah_value = request.headers.get(s_ah_name);
			if ((!req_ah_value) || (req_ah_value !== env.ah_value)) {
				return ret_on_err;
			}
			...
			switch (req_path) {
				case env.rd_path:
					...
					newRequest.headers.delete(s_ah_name);
					newRequest.headers.delete(env.padding_hname);
